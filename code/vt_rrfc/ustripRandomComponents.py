...
def randomGDS_dim(
  ...
  if rrfc.sim == 'EMX':
    ...
    density_target=random.randrange(20,45)

    pixel_numbers=(x_dim/rrfc.pixelSize)*(y_dim/rrfc.pixelSize)

    density=round((sum(sum(pixMap))*100/pixel_numbers))
    ...
    if density>density_target:
      for row in range(1,rows):
        for col in range(1,cols):
          if pixMap[row, col]==1:
            indicator=random.randrange(1,100)
            

            if indicator<= 100*(density-density_target)/density:
              pixMap[row,col]=0

    density_test=round(sum(sum(pixMap))*100/pixel_numbers)

    #pdb.set_trace()

    port1_pos=[0,random.randrange(2,rows-1)]
    port2_pos=[cols,random.randrange(2,rows-1)]
    port3_pos=[random.randrange(2,cols-1),0]
    port4_pos=[random.randrange(2,cols-1),rows]

    #pixMap[port1_pos[0],port1_pos[1]-1]=1
    #pixMap[port2_pos[0]-1,port2_pos[1]-1]=1
    #pixMap[port3_pos[0]-1,port3_pos[1]]=1
    #pixMap[port4_pos[0]-1,port4_pos[1]-1]=1

    pixMap[port1_pos[1]-1,port1_pos[0]]=1
    pixMap[port2_pos[1]-1,port2_pos[0]-1]=1
    pixMap[port3_pos[1],port3_pos[0]-1]=1
    pixMap[port4_pos[1]-1,port4_pos[0]-1]=1
    ...
    if rrfc.write == True:
      csvFile = '/home/local/millipede/hy7593/pixelatedRF_final/data/pixelMaps/' + rrfc.outF + ".csv"
      # Export Pixel Map file
      np.savetxt(csvFile, np.flipud(np.transpose(pixelMap)), fmt = '%d', delimiter = ",")
      gdsFile = '/home/local/millipede/hy7593/pixelatedRF_final/data/gds/' + rrfc.outF + ".gds"
      # Export GDS
  ...
  return portPos, x_total, y_total, csvFile, gdsFile, cellName, launch_l_pixels, port1_pos, port2_pos, port3_pos, port4_pos

def recreateGDS_file_2layer(rrfc):
  pixMap = np.flipud(np.loadtxt(rrfc.outF, delimiter=','))
  rows = int(np.size(pixMap, 0))
  cols = int(np.size(pixMap, 1))
  pixMap_top=np.zeros((rows,cols))     # initialization
  pixMap_bottom=np.zeros((rows,cols))  # initialization
  pixMap_gnd=np.zeros((rows,cols))  # initialization
  #pdb.set_trace()
  for row in range(0,rows):
    for col in range(0,cols):
      if int(pixMap[row][col])==0:
        pixMap_top[row][col]=0
        pixMap_bottom[row][col]=0
        pixMap_gnd[row][col]=0
      if int(pixMap[row][col])==1:
        pixMap_top[row][col]=1
        pixMap_bottom[row][col]=0
        pixMap_gnd[row][col]=0
      if int(pixMap[row][col])==2:
        pixMap_top[row][col]=0
        pixMap_bottom[row][col]=1
        pixMap_gnd[row][col]=0
      if int(pixMap[row][col])==3:
        pixMap_top[row][col]=1
        pixMap_bottom[row][col]=1
        pixMap_gnd[row][col]=0
      if int(pixMap[row][col])==4:
        pixMap_top[row][col]=0
        pixMap_bottom[row][col]=0
        pixMap_gnd[row][col]=1
      if int(pixMap[row][col])==5:
        pixMap_top[row][col]=1
        pixMap_bottom[row][col]=0
        pixMap_gnd[row][col]=1
      if int(pixMap[row][col])==6:
        pixMap_top[row][col]=0
        pixMap_bottom[row][col]=1
        pixMap_gnd[row][col]=1
      if int(pixMap[row][col])==7:
        pixMap_top[row][col]=1
        pixMap_bottom[row][col]=1
        pixMap_gnd[row][col]=1
  ...
  l_bottom = {"layer": 20, "datatype": 0}
  l_top = {"layer": 23, "datatype": 0}
  l_gnd = {"layer": 12, "datatype": 0}
  

  for x in range(0, cols):
    for y in range(0, rows):
      if pixMap_top[y,x]  == 1:
        rect_top = gdspy.Rectangle((0, 0), (rrfc.pixelSize, rrfc.pixelSize), **l_top).translate(x*rrfc.pixelSize,\
                y*rrfc.pixelSize)
        UNIT.add(rect_top)
      if pixMap_bottom[y,x] == 1:
        rect_bottom = gdspy.Rectangle((0, 0), (rrfc.pixelSize, rrfc.pixelSize), **l_bottom).translate(x*rrfc.pixelSize,\
                y*rrfc.pixelSize)
        UNIT.add(rect_bottom)
      if pixMap_gnd[y,x] == 1:
        rect_gnd = gdspy.Rectangle((0, 0), (rrfc.pixelSize, rrfc.pixelSize), **l_gnd).translate(x*rrfc.pixelSize,\
                y*rrfc.pixelSize)
        UNIT.add(rect_gnd)

  gdsFile = rrfc.gdsfile
  #pdb.set_trace()
  ##print(gdsFile) #debug 
  #if rrfc.view == True:
  #  gdspy.LayoutViewer(lib) 
...
def recreateGDS_file(rrfc):
  ...
  pixMap = np.flipud(np.loadtxt(rrfc.outF, delimiter=',')) # I don't know why for now, but it turned out the gds file generated will be a flipped version if not 'pre-flipped'
  rows = np.size(pixMap, 0)
  cols = np.size(pixMap, 1)
  if rrfc.sim == 'EMX':
    ...
    #l_bottom = {"layer": 10, "datatype": 0} #do not need bottom layer
    l_top = {"layer": 11, "datatype": 0}
    l_sources = {"layer": 5, "datatype": 0}
